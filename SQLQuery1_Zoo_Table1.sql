--CREATE DATABASE db_zoo; -- This Statement creates a table named db_zoo
USE db_zoo; /*this specifies wich db are we querying from.*/
...
INSERT INTO tbl_care
	...
	('care_4','clea up animal waste', 1),
	('care_5','move subject to exercise pen', 3),
	('care_6','drain and refil aquarium', 1),
...
--UPDATE
UPDATE tbl_species SET species_name = 'ghost bat' WHERE species_name = 'ghost bar';  -- this changes the text in the db
UPDATE tbl_persons SET persons_fname = 'mars' WHERE persons_fname = 'bob'; 


--SELECT
SELECT REPLACE(class_type, 'bird', 'birds') FROM tbl_class; -- This only temporarly replace the selected text with the new one, the text stored in the db doesn't change
SELECT UPPER(class_type) FROM tbl_class WHERE class_type = 'bird'; -- LOWER() , COUNT()
SELECT persons_fname, persons_lname, persons_contact FROM tbl_persons WHERE persons_id BETWEEN 3 AND 5; -- conjunction statement AND (they both has to be true)
SELECT persons_fname, persons_lname, persons_contact FROM tbl_persons WHERE persons_lname LIKE 'ke%'; -- using a wild card % to select all the names starting with ke.
SELECT persons_fname, persons_lname, persons_contact FROM tbl_persons WHERE persons_lname LIKE '_u%s'; -- _ is a wild card for the first character, folowed by u , folowed by a wild card for all characters and ends with s. 
SELECT persons_fname, persons_lname, persons_contact FROM tbl_persons WHERE persons_fname LIKE 'm%' ORDER BY persons_lname; -- ORDER BY is default as A to Z, use DESC for Z to A. 
SELECT persons_fname AS 'Firstname', persons_lname AS 'Lastname', persons_contact AS 'Phone:' FROM tbl_persons WHERE persons_fname LIKE 'm%' ORDER BY persons_lname; --alias names , AS 'TheNewName'.
SELECT * FROM tbl_persons;


--DELETE
DELETE FROM tbl_persons WHERE persons_lname = 'smith'; --this deletes the row that matches the condition. If WHERE is omitted, all the rows gets deleted. 
DROP TABLE tbl_persons; --deletes the entire table.
